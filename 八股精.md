...
obj.b() //'global'
```
箭头函数继承而来的this指向永远不变，对象obj的方法b是使用箭头函数定义的，这个函数中的this就永远指向它定义时所处的全局环境中的this

##### 闭包陷阱 循环中创建的多个函数共享了同一个循环变量的最终值
```js
for(var i=0;i<5;i++){
  setTimeout(function(){
    console.log(i)
...
```
实际结果是，它会快速地在控制台输出五个5
for循环是同步的，它会瞬间同步执行完毕
由于闭包，setTimeout的回调函数共享了同一个外部作用域的i
可以用let来声明变量

##### 在css选择器中，～是后续兄弟选择器
A~B 会选择所有与元素A拥有相同父元素，并且出现在A之后的所有B元素。
```html
<div>
  <p>段落 1</p>
  <span>这是一个 span</span>
...
```
比如h2~p，会选择在h2之后的所有p元素，即选中**段落2和段落3**
比如p~h2,会选择在p之后的所有h2元素，即选中**标题1和标题2**
